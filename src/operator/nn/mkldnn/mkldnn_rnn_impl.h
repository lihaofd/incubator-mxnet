...
template <typename DType>
void MKLDNNLstmForwardInference_r2l(DType* ws,
                                bool state_outputs,
                                int L,
                                int D,
                                const int T,
  ...
  const int b_size = 2 * H * 4 * D;
  const int cell_size = N * H * D;
  int w_size_0 = (I + H) * H * 4 * D;
  int w_size = (D * H + H) * H * 4 * D;
  ...
  std::vector<primitive> rnn_net_r2l_0;
  std::vector<primitive> rnn_net;
  std::vector<primitive> rnn_net_r2l;

  memory::dims src_layer_tz_0 = {T, N, I};
  memory::dims weights_layer_tz_0 = {1, 1, I, 4, H};  //  ldigo
  memory::dims weights_iter_tz_0 = {1, 1, H, 4, H};  //  ldigo
  memory::dims bias_tz_0 = {1, 1, 4, H};
  memory::dims dst_layer_tz_0 = {T, N, H};
  memory::dims src_iter_tz_0 = {1, 1, 2, N, H};  //  ldsnc
  memory::dims dst_iter_tz_0 = {1, 1, 2, N, H};  //  ldsnc

  memory::dims src_layer_tz = {T, N, H};  // T * N * H
  memory::dims weights_layer_tz = {L - 1, 1, H, 4, H};  //  ldigo
  memory::dims weights_iter_tz = {L - 1, 1, H, 4, H};  //  ldigo
  memory::dims bias_tz = {L - 1, 1, 4, H};
  memory::dims dst_layer_tz = {T, N, H};
  memory::dims src_iter_tz = {L - 1, 1, 2, N, H};  //  ldsnc
  memory::dims dst_iter_tz = {L - 1, 1, 2, N, H};  //  ldsnc

  memory::dims src_layer_r2l_tz_0 = {T, N, I};
  memory::dims weights_layer_r2l_tz_0 = {1, 1, I, 4, H};  //  ldigo
  memory::dims weights_iter_r2l_tz_0 = {1, 1, H, 4, H};  //  ldigo
  memory::dims bias_r2l_tz_0 = {1, 1, 4, H};
  memory::dims dst_layer_r2l_tz_0 = {T, N, H};
  memory::dims src_iter_r2l_tz_0 = {1, 1, 2, N, H};  //  ldsnc
  memory::dims dst_iter_r2l_tz_0 = {1, 1, 2, N, H};  //  ldsnc

  memory::dims src_layer_r2l_tz = {T, N, H};
  memory::dims weights_layer_r2l_tz = {L - 1, 1, H, 4, H};  //  ldigo
  memory::dims weights_iter_r2l_tz = {L - 1, 1, H, 4, H};  //  ldigo
  memory::dims bias_r2l_tz = {L - 1, 1, 4, H};
  memory::dims dst_layer_r2l_tz = {T, N, H};
  memory::dims src_iter_r2l_tz = {L - 1, 1, 2, N, H};  //  ldsnc
  memory::dims dst_iter_r2l_tz = {L - 1, 1, 2, N, H};  //  ldsnc
  
  std::vector<float> net_src_iter_0(1 * 2 * N * H, 0.0f);
  std::vector<float> net_src_r2l_iter_0(1 * 2 * N * H, 0.0f);
  std::vector<float> user_wei_layer_0(I * 4 * H, 0.0f);
  std::vector<float> user_wei_r2l_layer_0(I * 4 * H, 0.0f);
  std::vector<float> user_wei_iter_0(H * 4 * H, 0.0f);
  std::vector<float> user_wei_r2l_iter_0(H * 4 * H, 0.0f);
  std::vector<float> user_bias_0(4 * H, 0.0f);
  std::vector<float> user_r2l_bias_0(4 * H, 0.0f);

  std::vector<float> net_src_iter((L - 1) * 1 * 2 * N * H, 0.0f);
  std::vector<float> user_wei_layer((L - 1) * 1 * H * 4 * H, 0.0f);
  std::vector<float> user_wei_iter((L - 1) * 1 * H * 4 * H, 0.0f);
  std::vector<float> user_bias((L - 1) * 1 * 4 * H, 0.0f);
  std::vector<float> net_src_r2l_iter((L - 1) * 1 * 2 * N * H, 0.0f);
  std::vector<float> user_wei_r2l_layer((L - 1) * 1 * H * 4 * H, 0.0f);
  std::vector<float> user_wei_r2l_iter((L - 1) * 1 * H * 4 * H, 0.0f);
  std::vector<float> user_r2l_bias((L - 1) * 1 * 4 * H, 0.0f);

  std::vector<float> net_src_0(N * T * I, 0.0f);
  #pragma omp parallel for num_threads(omp_threads)
  for (size_t i = 0; i < N * T * I; i++) {
    net_src_0[i] = x.dptr_[i];
  }
  ...
  if (D == 2) {
    #pragma omp parallel for num_threads(omp_threads)
    for (int i = 0; i < N * H; i++) {
      net_src_r2l_iter_0[i] = hx[i + N * H];
      net_src_r2l_iter_0[i + N * H] = cx[i + N * H];
    }
  }
  ...
  if (D == 2) {
    ...
    for (int i = 0; i < I; i++) {
      for (int j = 0; j < H; j++) {
        user_wei_r2l_layer_0[i * 4 * H + j] = back_wx_f_0[j][i];
        user_wei_r2l_layer_0[i * 4 * H + H + j] = back_wx_i_0[j][i];
        user_wei_r2l_layer_0[i * 4 * H + 2 * H + j] = back_wx_o_0[j][i];
        user_wei_r2l_layer_0[i * 4 * H + 3 * H + j] = back_wx_g_0[j][i];
      }
    }
  }
  //  MKLDNN order is fiog, wx, wh, bx, bh order is ifgo
  ...
  if (D == 2) {
    ...
    for (int i = 0; i < H; i++) {
      for (int j = 0; j < H; j++) {
        user_wei_r2l_iter_0[i * 4 * H + j] = back_wh_f_0[j][i];
        user_wei_r2l_iter_0[i * 4 * H + H + j] = back_wh_i_0[j][i];
        user_wei_r2l_iter_0[i * 4 * H + 2 * H + j] = back_wh_o_0[j][i];
        user_wei_r2l_iter_0[i * 4 * H + 3 * H + j] = back_wh_g_0[j][i];
      }
    }
  }
  //  MKLDNN order is fiog, wx, wh, bx, bh order is ifgo
  ...
  if (D == 2) {
    #pragma omp parallel for num_threads(omp_threads)
    for (int j = 0; j < H; j++) {
      user_r2l_bias_0[j] = back_bx_0[1][j] + back_bh_0[1][j];
      user_r2l_bias_0[H + j] = back_bx_0[0][j] + back_bh_0[0][j];
      user_r2l_bias_0[2 * H + j] = back_bx_0[3][j] + back_bh_0[3][j];
      user_r2l_bias_0[3 * H + j] = back_bx_0[2][j] + back_bh_0[2][j];
    }
  }
  //  go to next L - 1 layers param collection
  w_ptr += w_size_0;
  b_ptr += b_size;
  if (L > 1) {
    if (state_outputs) {
    ...
    for (int l = 0; l < L - 1; l++) {
      #pragma omp parallel for num_threads(omp_threads)
      for (int i = 0; i < N * H; i++) {
        net_src_iter[l * 2 * N * H + i] = hx[l * D * N * H + i];
        net_src_iter[l * 2 * N * H + i + N * H] = cx[l * D * N * H + i];
      }
      if (D == 2) {
        #pragma omp parallel for num_threads(omp_threads)
        for (int i = 0; i < N * H; i++) {
          net_src_r2l_iter[l * 2 * N * H + i] = hx[l * D * N * H + i + N * H];
          net_src_r2l_iter[l * 2 * N * H + i + N * H] = cx[l * D * N * H + i + N * H];
        }
      }
      back_w_ptr = w_ptr;
      back_b_ptr = b_ptr;
      const Tensor<cpu, 2, DType> wx_i(w_ptr, Shape2(H, D * H));
      const Tensor<cpu, 2, DType> wh_i(w_ptr + D * H * H * 4, Shape2(H, H));
      const Tensor<cpu, 2, DType> wx_f(w_ptr + H * D * H, Shape2(H, D * H));
      const Tensor<cpu, 2, DType> wh_f(w_ptr + D * H * H * 4 + H * H, Shape2(H, H));
      const Tensor<cpu, 2, DType> wx_g(w_ptr + H * D * H * 2, Shape2(H, D * H));
      const Tensor<cpu, 2, DType> wh_g(w_ptr + D * H * H * 4 + H * H * 2, Shape2(H, H));
      const Tensor<cpu, 2, DType> wx_o(w_ptr + H * D * H * 3, Shape2(H, D * H));
      const Tensor<cpu, 2, DType> wh_o(w_ptr + D * H * H * 4 + H * H * 3, Shape2(H, H));
      if (D == 2) {
        back_w_ptr = w_ptr + 4 * H * (D * H + H);
        back_b_ptr = b_ptr + 4 * H * 2;
      }
      const Tensor<cpu, 2, DType> back_wx_i(back_w_ptr , Shape2(H, D * H));
      const Tensor<cpu, 2, DType> back_wh_i(back_w_ptr + D * H * H * 4, Shape2(H, H));
      const Tensor<cpu, 2, DType> back_wx_f(back_w_ptr + H * D * H, Shape2(H, D * H));
      const Tensor<cpu, 2, DType> back_wh_f(back_w_ptr + D * H * H * 4 + H * H, Shape2(H, H));
      const Tensor<cpu, 2, DType> back_wx_g(back_w_ptr + H * D * H * 2, Shape2(H, D * H));
      const Tensor<cpu, 2, DType> back_wh_g(back_w_ptr + D * H * H * 4 + H * H * 2, Shape2(H, H));
      const Tensor<cpu, 2, DType> back_wx_o(back_w_ptr + H * D * H * 3, Shape2(H, D * H));
      const Tensor<cpu, 2, DType> back_wh_o(back_w_ptr + D * H * H * 4 + H * H * 3, Shape2(H, H));
      ...
      for (int i = 0; i < H; i++) {
        for (int j = 0; j < H; j++) {
          user_wei_layer[l * H * 4 * H + i * 4 * H + j] = wx_f[j][i];
          user_wei_layer[l * H * 4 * H + i * 4 * H + H + j] = wx_i[j][i];
          user_wei_layer[l * H * 4 * H + i * 4 * H + 2 * H + j] = wx_o[j][i];
          user_wei_layer[l * H * 4 * H + i * 4 * H + 3 * H + j] = wx_g[j][i];
        }
      }
      if (D == 2) {
        #pragma omp parallel for num_threads(omp_threads)
        for (int i = 0; i < H; i++) {
          for (int j = 0; j < H; j++) {
            user_wei_r2l_layer[l * H * 4 * H + i * 4 * H + j] = back_wx_f[j][i];
            user_wei_r2l_layer[l * H * 4 * H + i * 4 * H + H + j] = back_wx_i[j][i];
            user_wei_r2l_layer[l * H * 4 * H + i * 4 * H + 2 * H + j] = back_wx_o[j][i];
            user_wei_r2l_layer[l * H * 4 * H + i * 4 * H + 3 * H + j] = back_wx_g[j][i];
          }
        }
      }
      //  MKLDNN order is fiog, wx, wh, bx, bh order is ifgo
      ...
      for (int i = 0; i < H; i++) {
        for (int j = 0; j < H; j++) {
          user_wei_iter[l * H * 4 * H + i * 4 * H + j] = wh_f[j][i];
          user_wei_iter[l * H * 4 * H + i * 4 * H + H + j] = wh_i[j][i];
          user_wei_iter[l * H * 4 * H + i * 4 * H + 2 * H + j] = wh_o[j][i];
          user_wei_iter[l * H * 4 * H + i * 4 * H + 3 * H + j] = wh_g[j][i];
        }
      }
      if (D == 2) {
        #pragma omp parallel for num_threads(omp_threads)
        for (int i = 0; i < H; i++) {
          for (int j = 0; j < H; j++) {
            user_wei_r2l_iter[l * H * 4 * H + i * 4 * H + j] = back_wh_f[j][i];
            user_wei_r2l_iter[l * H * 4 * H + i * 4 * H + H + j] = back_wh_i[j][i];
            user_wei_r2l_iter[l * H * 4 * H + i * 4 * H + 2 * H + j] = back_wh_o[j][i];
            user_wei_r2l_iter[l * H * 4 * H + i * 4 * H + 3 * H + j] = back_wh_g[j][i];
          }
        }
      }
      //  MKLDNN order is fiog, wx, wh, bx, bh order is ifgo
      #pragma omp parallel for num_threads(omp_threads)
      for (int j = 0; j < H; j++) {
        user_bias[l * H * 4 + j] = bx[1][j] + bh[1][j];
        user_bias[l * H * 4 + H + j] = bx[0][j] + bh[0][j];
        user_bias[l * H * 4 + 2 * H + j] = bx[3][j] + bh[3][j];
        user_bias[l * H * 4 + 3 * H + j] = bx[2][j] + bh[2][j];
      }
      if (D == 2) {
        #pragma omp parallel for num_threads(omp_threads)
        for (int j = 0; j < H; j++) {
          user_r2l_bias[l * H * 4 + j] = back_bx[1][j] + back_bh[1][j];
          user_r2l_bias[l * H * 4 + H + j] = back_bx[0][j] + back_bh[0][j];
          user_r2l_bias[l * H * 4 + 2 * H + j] = back_bx[3][j] + back_bh[3][j];
          user_r2l_bias[l * H * 4 + 3 * H + j] = back_bx[2][j] + back_bh[2][j];
        }
      }
      w_ptr = w_ptr + w_size;
      b_ptr = b_ptr + b_size;
  ...
  if (L > 1) {
    ...
    if (state_outputs) {
      for (int l = 0; l < L - 1; l++) {
        #pragma omp parallel for num_threads(omp_threads)
        for (size_t n = 0; n < N * H; n++) {
          hy_ptr[l * D * N * H + n] = dst_hcy[l * 2 * N * H + n];
          cy_ptr[l * D * N * H + n] = dst_hcy[l * 2 * N * H + n + N * H];
        }
      }
    }
  }
//  r2l
  if (D == 2) {    
    auto user_src_layer_r2l_md_0 = mkldnn::memory::desc(
        { src_layer_r2l_tz_0 }, mkldnn::memory::data_type::f32,
        mkldnn::memory::format::tnc);

    auto user_src_iter_r2l_md_0 = mkldnn::memory::desc(
        { src_iter_r2l_tz_0 }, mkldnn::memory::data_type::f32,
        mkldnn::memory::format::ldsnc);

    auto user_wei_layer_r2l_md_0 = mkldnn::memory::desc(
        { weights_layer_r2l_tz_0 }, mkldnn::memory::data_type::f32,
        mkldnn::memory::format::ldigo);

    auto user_wei_iter_r2l_md_0 = mkldnn::memory::desc(
        { weights_iter_r2l_tz_0 }, mkldnn::memory::data_type::f32,
        mkldnn::memory::format::ldigo);

    auto user_bias_r2l_md_0 = mkldnn::memory::desc({ bias_r2l_tz_0 },
        mkldnn::memory::data_type::f32, mkldnn::memory::format::ldgo);

    auto dst_layer_r2l_md_0 = mkldnn::memory::desc(
        { dst_layer_r2l_tz_0 }, mkldnn::memory::data_type::f32,
        mkldnn::memory::format::tnc);

    auto dst_iter_r2l_md_0 = mkldnn::memory::desc(
        { dst_iter_r2l_tz_0 }, mkldnn::memory::data_type::f32,
        mkldnn::memory::format::ldsnc);

    auto user_src_layer_r2l_memory_0 = mkldnn::memory(
        { user_src_layer_r2l_md_0, cpu_engine }, net_src_0.data());

    auto user_wei_layer_r2l_memory_0
        = mkldnn::memory({ user_wei_layer_r2l_md_0, cpu_engine },
          user_wei_r2l_layer_0.data());

    auto user_wei_iter_r2l_memory_0
        = mkldnn::memory({ user_wei_iter_r2l_md_0, cpu_engine },
        user_wei_r2l_iter_0.data());

    auto user_bias_r2l_memory_0 = mkldnn::memory(
        { user_bias_r2l_md_0, cpu_engine }, user_r2l_bias_0.data());

    auto user_src_iter_r2l_memory_0 = mkldnn::memory(
        { user_src_iter_r2l_md_0, cpu_engine }, net_src_r2l_iter_0.data());

    rnn_cell::desc cell_r2l_0(algorithm::vanilla_lstm);
    rnn_forward::desc layer_r2l_desc_0(prop_kind::forward_inference, cell_r2l_0,
        rnn_direction::unidirectional_right2left, user_src_layer_r2l_md_0,
        user_src_iter_r2l_md_0, user_wei_layer_r2l_md_0, user_wei_iter_r2l_md_0,
        user_bias_r2l_md_0, dst_layer_r2l_md_0, dst_iter_r2l_md_0);
    auto prim_r2l_desc_0
        = mkldnn::rnn_forward::primitive_desc(layer_r2l_desc_0, cpu_engine);
    auto dst_layer_r2l_memory_0
        = mkldnn::memory(prim_r2l_desc_0.dst_layer_primitive_desc());

    auto dst_iter_r2l_memory_0
        = mkldnn::memory(prim_r2l_desc_0.dst_iter_primitive_desc());

    rnn_net_r2l_0.push_back(
        rnn_forward(prim_r2l_desc_0, user_src_layer_r2l_memory_0,
                    user_src_iter_r2l_memory_0, user_wei_layer_r2l_memory_0,
                    user_wei_iter_r2l_memory_0, user_bias_r2l_memory_0,
                    dst_layer_r2l_memory_0, dst_iter_r2l_memory_0, null_memory_));

    stream(stream::kind::eager).submit(rnn_net_r2l_0).wait();
    float* dst_r2l_y_0 = reinterpret_cast<float *> (dst_layer_r2l_memory_0.get_data_handle());
    float* dst_r2l_hcy_0 = reinterpret_cast<float *> (dst_iter_r2l_memory_0.get_data_handle());
    if (L == 1) {
      #pragma omp parallel for num_threads(omp_threads)
      for (int i = 0; i < T; i++) {
        for (int j = 0; j < N; j++) {
          for (int k = 0; k < H; k++) {
            y[i][j][k + H] = dst_r2l_y_0[i * N * H + j * H + k];
          }      
        }
      }
    }
    if (state_outputs) {
      #pragma omp parallel for num_threads(omp_threads)
      for (size_t n = 0; n < N * H; n++) {
        hy_ptr[n + N * H] = dst_r2l_hcy_0[n];
        cy_ptr[n + N * H] = dst_r2l_hcy_0[n + N * H];
      }
    }
    if (L > 1) {
      auto user_src_layer_r2l_md = mkldnn::memory::desc(
        { src_layer_r2l_tz }, mkldnn::memory::data_type::f32,
        mkldnn::memory::format::tnc);

      auto user_src_iter_r2l_md = mkldnn::memory::desc(
        { src_iter_r2l_tz }, mkldnn::memory::data_type::f32,
        mkldnn::memory::format::ldsnc);

      auto user_wei_layer_r2l_md = mkldnn::memory::desc(
          { weights_layer_r2l_tz }, mkldnn::memory::data_type::f32,
          mkldnn::memory::format::ldigo);

      auto user_wei_iter_r2l_md = mkldnn::memory::desc(
          { weights_iter_r2l_tz }, mkldnn::memory::data_type::f32,
          mkldnn::memory::format::ldigo);

      auto user_bias_r2l_md = mkldnn::memory::desc({ bias_r2l_tz },
          mkldnn::memory::data_type::f32, mkldnn::memory::format::ldgo);

      auto dst_layer_r2l_md = mkldnn::memory::desc(
          { dst_layer_r2l_tz }, mkldnn::memory::data_type::f32,
          mkldnn::memory::format::tnc);

      auto dst_iter_r2l_md = mkldnn::memory::desc(
          { dst_iter_r2l_tz }, mkldnn::memory::data_type::f32,
           mkldnn::memory::format::ldsnc);

      auto user_wei_layer_r2l_memory
          = mkldnn::memory({ user_wei_layer_r2l_md, cpu_engine },
          user_wei_r2l_layer.data());

      auto user_wei_iter_r2l_memory
          = mkldnn::memory({ user_wei_iter_r2l_md, cpu_engine },
          user_wei_r2l_iter.data());

      auto user_bias_r2l_memory = mkldnn::memory(
          { user_bias_r2l_md, cpu_engine }, user_r2l_bias.data());
      auto user_src_iter_r2l_memory = mkldnn::memory(
          { user_src_iter_r2l_md, cpu_engine }, net_src_r2l_iter.data());
      rnn_cell::desc cell_r2l(algorithm::vanilla_lstm);
      rnn_forward::desc layer_desc_r2l(prop_kind::forward_inference, cell_r2l,
          rnn_direction::unidirectional_right2left, user_src_layer_r2l_md,
          user_src_iter_r2l_md, user_wei_layer_r2l_md, user_wei_iter_r2l_md,
          user_bias_r2l_md, dst_layer_r2l_md, dst_iter_r2l_md);
      auto prim_desc_r2l
          = mkldnn::rnn_forward::primitive_desc(layer_desc_r2l, cpu_engine);
      auto dst_layer_r2l_memory
          = mkldnn::memory(prim_desc_r2l.dst_layer_primitive_desc());

      auto dst_iter_r2l_memory
          = mkldnn::memory(prim_desc_r2l.dst_iter_primitive_desc());
      rnn_net_r2l.push_back(
          rnn_forward(prim_desc_r2l, dst_layer_r2l_memory_0,
                      user_src_iter_r2l_memory, user_wei_layer_r2l_memory,
                      user_wei_iter_r2l_memory, user_bias_r2l_memory,
                      dst_layer_r2l_memory, dst_iter_r2l_memory, null_memory_));

      stream(stream::kind::eager).submit(rnn_net_r2l).wait();
      float* dst_r2l_y = reinterpret_cast<float *> (dst_layer_r2l_memory.get_data_handle());
      float* dst_r2l_hcy = reinterpret_cast<float *> (dst_iter_r2l_memory.get_data_handle());
      #pragma omp parallel for num_threads(omp_threads)
      for (int i = 0; i < T; i++) {
        for (int j = 0; j < N; j++) {
          for (int k = 0; k < H; k++) {
            y[i][j][k + H] = dst_r2l_y[i * N * H + j * H + k];
          }      
        }
      }
      if (state_outputs) {
        for (int l = 0; l < L - 1; l++) {
          #pragma omp parallel for num_threads(omp_threads)
          for (size_t n = 0; n < N * H; n++) {
            hy_ptr[l * D * N * H + n + N * H] = dst_r2l_hcy[l * 2 * N * H + n];
            cy_ptr[l * D * N * H + n + N * H] = dst_r2l_hcy[l * 2 * N * H + n + N * H];
          }
        }
      }
    }
